...
class Analyse():
    ...
    def cleanData(self):
        ...
        self.df.loc[self.df['City'].isin(
            ['\\\\xc2\\\\xa0Noida', '\\xc2\\xa0Noida']), 'City'] = 'Noida'
        self.df.loc[self.df['City'].isin(
            ['\\\\xc2\\\\xa0Bangalore', '\\xc2\\xa0Bangalore', 'Bangalore']), 'City'] = 'Bengaluru'
        self.df.loc[self.df['City'].isin(
            ['\\\\xc2\\\\xa0New Delhi', '\\xc2\\xa0New Delhi']), 'City'] = 'New Delhi'
        self.df.loc[self.df['City'].isin(
            ['\\\\xc2\\\\xa0Gurgaon', 'Gurugram']), 'City'] = 'Gurgaon'
        self.df.loc[self.df['City'].isin(
            ['\\\\xc2\\\\xa0Mumbai', '\\xc2\\xa0Mumbai']), 'City'] = 'Mumbai'

        self.df.loc[self.df['IndustryVertical'] == "\\\\xc2\\\\xa0News Aggregator mobile app",
                    'IndustryVertical'] = 'News Aggregator mobile app'
        self.df.loc[self.df['IndustryVertical'] == "\\\\xc2\\\\xa0Online Jewellery Store",
                    'IndustryVertical'] = 'Online Jewellery Store'
        self.df.loc[self.df['IndustryVertical'] == "\\\\xc2\\\\xa0Fashion Info Aggregator App",
                    'IndustryVertical'] = 'Fashion Info Aggregator App'
        self.df.loc[self.df['IndustryVertical'] == "\\\\xc2\\\\xa0Online Study Notes Marketplace",
                    'IndustryVertical'] = 'Online Study Notes Marketplace'
        self.df.loc[self.df['IndustryVertical'] == "\\\\xc2\\\\xa0Warranty Programs Service Administration",
                    'IndustryVertical'] = 'Warranty Programs Service Administration'
        self.df.loc[self.df['IndustryVertical'] ==
                    "\\\\xc2\\\\xa0Pre-School Chain", 'IndustryVertical'] = 'Pre-School Chain'
        self.df.loc[self.df['IndustryVertical'] == "\\\\xc2\\\\xa0Premium Loyalty Rewards Point Management",
                    'IndustryVertical'] = 'Premium Loyalty Rewards Point Management'
        self.df.loc[self.df['IndustryVertical'] == "\\\\xc2\\\\xa0Contact Center Software Platform",
                    'IndustryVertical'] = 'Contact Center Software Platform'
        self.df.loc[self.df['IndustryVertical'] == "\\\\xc2\\\\xa0Casual Dining restaurant Chain",
                    'IndustryVertical'] = 'Casual Dining restaurant Chain'
        self.df.loc[self.df['IndustryVertical'] == "\\\\xc2\\\\xa0Online Grocery Delivery",
                    'IndustryVertical'] = 'Online Grocery Delivery'
        self.df.loc[self.df['IndustryVertical'] == "Online home d\\\\xc3\\\\xa9cor marketplace",
                    'IndustryVertical'] = 'Online home decor marketplace'
        self.df.loc[self.df['IndustryVertical'].isin(
            ["Fin-Tech"]), 'IndustryVertical'] = 'FinTech'

        self.df.loc[self.df['InvestorsName'].isin(['Undisclosed investors', 'Undisclosed', 'undisclosed investors',
                                                   'Undisclosed Investor', 'Undisclosed investors']), 'InvestorsName'] = 'Undisclosed Investors'
        self.df.loc[self.df['InvestorsName'] ==
                    "\\\\xc2\\\\xa0Tiger Global", 'InvestorsName'] = 'Tiger Global'
        self.df.loc[self.df['InvestorsName'] ==
                    "\\\\xc2\\\\xa0IndianIdeas.com", 'InvestorsName'] = 'IndianIdeas'
        self.df.loc[self.df['InvestorsName'] == "\\\\xc2\\\\xa0IvyCap Ventures, Accel Partners, Dragoneer Investment Group",
                    'InvestorsName'] = 'IvyCap Ventures, Accel Partners, Dragoneer Investment Group'
        self.df.loc[self.df['InvestorsName'] ==
                    "\\\\xc2\\\\xa0Goldman Sachs", 'InvestorsName'] = 'Goldman Sachs'

        self.df.drop([2602, 2603, 2604, 2605, 2606, 2607,
                      2608, 2609, 2610, 2611], inplace=True)
        self.df.reset_index(drop=True, inplace=True)

        for i in range(0, len(self.df["AmountInUSD"])):
            self.df["AmountInUSD"][i] = re.sub(
                '\D', "", str(self.df["AmountInUSD"][i]))
        self.df["AmountInUSD"] = pd.to_numeric(self.df["AmountInUSD"])

        for i in range(0, len(self.df["StartupName"])):
            self.df["StartupName"][i] = re.sub(
                'xc2xa0', "", str(self.df["StartupName"][i]))

        self.df['InvestmentType'] = self.df['InvestmentType'].apply(
    ...
    def getStartupFunding(self, n):
        return self.df

    def getFundvsStartup(self, n):
        totalFunding = self.df['StartupName'].value_counts()
        company = self.df.groupby(['StartupName'])['AmountInUSD'].sum()
        allMerge = pd.merge(totalFunding,company,on=totalFunding.index)
        allMerge.rename(columns={'key_0':"StartupName",'StartupName':'TotalTimesFunding','AmountInUSD':'TotalFund'},inplace=True)
